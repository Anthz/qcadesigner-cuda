		...
		while (!stable && iteration < max_iterations_per_sample)
		{
			...
			for (icLayers = 0; icLayers < number_of_cell_layers; icLayers++)
			{
				...
				/*#endif*/
				{
					...
					/*  if(iteration == 1){
					printf("cella: %d vicini ",icLayers*number_of_cells_in_layer[icLayers]+icCellsInLayer);
					for (q = 0; q < current_cell_model->number_of_neighbours; q++)
					printf("%d ek:%e clock:%d\t",q,((bistable_model *)cell->cell_model)->Ek[q],cell->cell_options.clock,((bistable_model *)((bistable_model *)cell->cell_model)->neighbours[q]->cell_model)->polarization);
					/*          printf("\n");
					}*/
